...
class TradeRange:
    """
    mc: 用于限制内层策略/执行器的交易时间范围，确保内层只在外层指定的时间窗口内执行决策
    （例如只在某天的 9:30-14:30 交易，或只在前 50 步执行）。

        设计动机与应用场景
    1. 嵌套执行中的时间控制

    在多级嵌套执行（例如日线策略 → 分钟线策略 → 订单执行器）中，外层策略可能希望内层只在特定时段交易。
    例如：
    外层日线策略下达决策："在今天 9:30-11:30 期间分批买入"
    内层分钟线执行器需要知道这个时间窗口，不能在 11:30 之后继续执行
    2. 分段交易（Split Execution）

    大单需要分拆到多个时间段逐步执行，避免市场冲击。
    外层决策指定每个子决策的执行时间范围（例如第 1-10 步、第 11-20 步等）。
    3. 日内交易时段限制

    某些策略只允许在特定时段交易（例如开盘/收盘前后、避开午休）。
    TradeRangeByTime 可以精确控制"只在 9:30-10:00 和 14:00-14:30 交易"。
    """
    @abstractmethod
